...
export default function KartuStokList({
  ...
  // Bahan Baku & BSJ dipisah jadi 2 kelompok dengan pembatas visual (sama
  // pola dengan halaman Bahan Baku pusat) -- sebelumnya kecampur satu daftar
  // urut abjad, susah bedain kembaran BSJ dari bahan baku asli sekilas.
  ...
  function renderRow(item: KartuStokRow) {
      ...
      <div className="mt-3 space-y-2">
        {filtered.length > 0 ? (
          <>
            ...
            {showGroupHeadings && renderDivider("Bahan Baku")}
            {ingredientItems.map(renderRow)}
            {showGroupHeadings && renderDivider("Bahan Setengah Jadi")}
            {semiFinishedItems.map(renderRow)}
          </>
        ) : (
