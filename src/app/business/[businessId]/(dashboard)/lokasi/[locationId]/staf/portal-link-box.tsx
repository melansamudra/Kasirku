...
import { regeneratePortalSlug } from "./actions";
...
export default function PortalLinkBox({
  ...
  async function handleRegenerate() {
    setError(null);
    setPending(true);
    const result = await regeneratePortalSlug(businessId, locationId);
    setPending(false);
  ...
  return (
    <div className="mt-4 rounded-xl bg-white shadow-sm p-4">
      <h2 className="text-sm font-semibold text-zinc-900">🔑 Link Portal Lokasi (Scan)</h2>
      <p className="mt-1 text-[11px] text-zinc-400">
        Bagikan ke staf yang sudah punya PIN (print jadi poster/QR, atau kirim lewat grup
        WhatsApp) — mereka scan, pilih nama, masukin PIN, lalu bisa Kirim/Terima Barang/Stok
        Opname dari 1 tempat tanpa login penuh.
      </p>
      <div className="mt-2 flex items-center gap-2 rounded-lg border border-zinc-200 bg-zinc-50 px-3 py-2.5">
        ...
        <Link
          href={`/business/${businessId}/lokasi/${locationId}/staf/print-qr`}
          target="_blank"
          className="shrink-0 rounded-lg border border-zinc-200 bg-white px-2.5 py-1 text-[11px] font-medium text-zinc-600 transition-colors hover:border-brand-300 hover:text-brand-700"
        >
          🖨️ Cetak QR
