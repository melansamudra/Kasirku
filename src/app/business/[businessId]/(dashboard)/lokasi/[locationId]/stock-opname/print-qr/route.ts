...
export async function GET(
  ...
  const [{ data: biz }, { data: location }] = await Promise.all([
    supabase.from("businesses").select("name, stock_opname_slug").eq("id", businessId).single(),
    supabase.from("stock_locations").select("name").eq("id", locationId).eq("business_id", businessId).maybeSingle(),
  ]);

  if (!biz?.stock_opname_slug) {
    return new NextResponse("Link stok opname belum tersedia.", {
      status: 200,
...
<head>
  <meta charset="UTF-8" />
  <title>QR Stok Opname — ${escapeHtml(biz.name)}</title>
  <style>
...
<body>
  <div class="card">
    ...
    <p class="sub">Scan buat isi stok opname harian — khusus ${escapeHtml(locationName)}</p>
