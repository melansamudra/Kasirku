...
import type { PostAmortizationResult } from "./actions";

function formatRupiah(value: number) {
  return `Rp${Math.round(value).toLocaleString("id-ID")}`;
}

export default function PostAmortizationButton({
  action,
  period,
  estimatedAmount,
}: {
  action: (period: string) => Promise<PostAmortizationResult>;
  period: string;
  ...
  if (!confirming) {
    return (
      ...
      >
        {estimatedAmount > 0
          ? `📅 Posting Cicilan Bulan Ini (${formatRupiah(estimatedAmount)})`
  ...
  return (
    <div className="space-y-2 rounded-xl border border-brand-200 bg-brand-50 p-3 text-xs">
      <p className="text-brand-700">
        Posting cicilan {formatRupiah(estimatedAmount)} untuk periode {period.slice(0, 7)} ke jurnal
        (Beban terkait / Biaya Dibayar Dimuka)? Bulan ini tidak bisa diposting dua kali.
      </p>
