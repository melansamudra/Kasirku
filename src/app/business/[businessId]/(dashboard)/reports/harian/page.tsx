...
export default async function ReportsHarianPage({
  ...
  const dayList = Array.from(dayMap.entries())
    .map(([date, v]) => {
      ...
      // Total Beban -- BUKAN estimasi, ini biaya NYATA yang sudah terjadi hari
      // itu: kas yang beneran keluar + pembelian yang masih hutang (biayanya
      // sudah terjadi walau uangnya belum keluar). Beda dari Total Pengeluaran
      // di atas yang murni kas aktual (cash basis). Estimasi Gaji SENGAJA
      // tidak diikutkan ke sini -- itu beneran cuma perkiraan kasar, bukan
      // biaya pasti kayak dua komponen di atas.
      const totalBeban = totalPengeluaran + v.hutangBertambah;
      return {
        date,
        ...v,
        totalPendapatan,
  ...
  const totals = dayList.reduce(
    (acc, d) => ({
      ...
      totalBeban: acc.totalBeban + d.totalBeban,
    }),
    {
      ...
      hutangBertambah: 0, hutangDibayar: 0, labaBersih: 0, estimasiGaji: 0, totalBeban: 0,
    },
  );
  const totalsPersenBeban = totals.totalPendapatan > 0 ? Math.round((totals.totalPengeluaran / totals.totalPendapatan) * 100) : 0;
  const totalsPersenTotalBeban =
    totals.totalPendapatan > 0 ? Math.round((totals.totalBeban / totals.totalPendapatan) * 100) : 0;
  ...
  const hasTotalBeban = dayList.some((d) => d.hutangBertambah > 0);
              ...
              <table className="w-full text-sm print:text-[9px]">
                <thead className="border-b border-zinc-100 bg-zinc-50">
                  <tr className="text-left text-[10px] font-semibold uppercase tracking-wide text-zinc-400 print:text-[8px]">
                    ...
                    {hasTotalBeban && <th className="px-3 py-3 text-right print:px-1 print:py-1">Total Beban</th>}
                    {hasTotalBeban && <th className="px-3 py-3 text-right print:px-1 print:py-1">% Total Beban</th>}
                ...
                <tbody>
                  {dayList.map((d, i) => (
                    <tr key={d.date} className={`border-b border-zinc-50 last:border-0 ${i % 2 === 0 ? "" : "bg-zinc-50/40"}`}>
                      ...
                      {hasTotalBeban && <td className="px-3 py-2.5 text-right text-xs font-bold text-zinc-600 print:px-1 print:py-0.5">{fmt(d.totalBeban)}</td>}
                      {hasTotalBeban && <td className="px-3 py-2.5 text-right text-xs text-zinc-500 print:px-1 print:py-0.5">{d.persenTotalBeban}%</td>}
                ...
                <tfoot className="border-t-2 border-zinc-200 bg-zinc-50">
                  <tr>
                    ...
                    {hasTotalBeban && <td className="px-3 py-3 text-right text-xs font-bold text-zinc-600 print:px-1 print:py-1">{fmt(totals.totalBeban)}</td>}
                    {hasTotalBeban && <td className="px-3 py-3 text-right text-xs font-bold text-zinc-500 print:px-1 print:py-1">{totalsPersenTotalBeban}%</td>}
          ...
          <p className="mt-3 text-center text-[11px] text-zinc-400 print:hidden">
            ...
            {hasHutang && (
              <>
                ...
                hutang baru), bukan bagian dari Total Pengeluaran (karena uangnya belum keluar).
              </>
            )}
            {hasTotalBeban && (
              <>
                {" "}Kolom &quot;Total Beban&quot; &amp; &quot;% Total Beban&quot; = Total Pengeluaran +
                Pembelian Hutang — ini biaya NYATA yang sudah terjadi hari itu (bukan estimasi), cuma sebagian
                uangnya belum keluar karena masih hutang. Beda dari &quot;Total Pengeluaran&quot;/&quot;%
                Beban&quot; di atas yang murni kas aktual. Sengaja TIDAK mengurangi Laba Bersih di atas, biar
                Laba Bersih tetap cerminan kas yang beneran sudah keluar/masuk.
              </>
            )}
            {hasEstimasiGaji && (
              <>
                ...
                izin-telat, dan sengaja TIDAK diikutkan ke &quot;Total Beban&quot;/Total Pengeluaran/Laba Bersih
                karena ini beneran cuma perkiraan (bukan biaya pasti kayak Pembelian Hutang), dan gaji
