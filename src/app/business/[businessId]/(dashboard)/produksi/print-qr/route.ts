...
export async function GET(
  ...
  const { data: biz } = await supabase
    .from("businesses")
    .select("name, production_scan_slug")
    .eq("id", businessId)
    .single();

  if (!biz?.production_scan_slug) {
    return new NextResponse("Link scan produksi belum tersedia.", {
      status: 200,
      headers: { "Content-Type": "text/plain" },
    });
  }

  const url = `${SITE_URL}/produksi-scan/${biz.production_scan_slug}`;
...
<head>
  <meta charset="UTF-8" />
  <title>QR Catat Produksi — ${escapeHtml(biz.name)}</title>
  <style>
...
<body>
  <div class="card">
    ...
    <p class="sub">Scan buat catat produksi bahan setengah jadi (tanpa login)</p>
