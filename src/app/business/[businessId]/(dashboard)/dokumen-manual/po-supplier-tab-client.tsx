...
import PoSupplierManualForm from "../lokasi/[locationId]/dokumen-manual/po-supplier-form";
...
import { getManualPurchaseOrderDetail } from "./actions";
import type { HistoryEntry } from "./surat-jalan-tab-client";
...
export default function PoSupplierTabClient({
  businessId,
  businessName,
  history,
}: {
  ...
  async function handleHistoryClick(id: string) {
    setLoadingId(id);
    const detail = await getManualPurchaseOrderDetail(businessId, id);
    setLoadingId(null);
  ...
  return (
    <>
      <PoSupplierManualForm businessId={businessId} locationId={null} onSuccess={handleSuccess} />
      ...
      <div className="mt-6">
        <h2 className="mb-2 text-sm font-semibold text-zinc-900">Riwayat PO Supplier</h2>
        {history.length === 0 ? (
          <p className="rounded-xl border border-dashed border-zinc-200 px-4 py-6 text-center text-xs text-zinc-400">
            Belum ada PO Supplier manual yang dibuat.
          </p>
        ) : (
