...
import { getManualPurchaseRequestDetail } from "./actions";
...
export default function PermintaanBarangTabClient({
  businessId,
  businessName,
  history,
}: {
  ...
  async function handleHistoryClick(id: string) {
    setLoadingId(id);
    const detail = await getManualPurchaseRequestDetail(businessId, id);
    setLoadingId(null);
  ...
  return (
    <>
      <PermintaanBarangManualForm businessId={businessId} locationId={null} onSuccess={handleSuccess} />
      ...
      <div className="mt-6">
        <h2 className="mb-2 text-sm font-semibold text-zinc-900">Riwayat Permintaan Barang</h2>
        {history.length === 0 ? (
          <p className="rounded-xl border border-dashed border-zinc-200 px-4 py-6 text-center text-xs text-zinc-400">
            Belum ada Permintaan Barang manual yang dibuat.
          </p>
        ) : (
