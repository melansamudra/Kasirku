...
import type { AddReceivablePaymentState } from "./actions";
...
export default function AddPaymentForm({
  ...
  sisaPiutang: number;
  action: (
    state: AddReceivablePaymentState,
    formData: FormData,
  ) => Promise<AddReceivablePaymentState>;
}) {
  const router = useRouter();
  const [open, setOpen] = useState(false);
  const [amount, setAmount] = useState(String(sisaPiutang));
      ...
      <div className="grid grid-cols-2 gap-2">
        ...
        <div>
          <label className="mb-1 block text-xs font-medium text-brand-800">
            Jumlah Diterima (sisa {sisaPiutang.toLocaleString("id-ID")})
          </label>
          <input
            type="number"
            min="0"
