...
export default function ImportMokaForm({
  ...
  if (previewState.preview) {
    const { willImport, willSkip, skipReasons, txJson } = previewState.preview;
    return (
      <div className="space-y-4">
        {/* Ringkasan */}
        <div className="grid grid-cols-2 gap-2">
          ...
          <div className={`rounded-xl px-3 py-2.5 text-center ${willSkip > 0 ? "bg-amber-50" : "bg-zinc-50"}`}>
            ...
            <p className={`text-xs ${willSkip > 0 ? "text-amber-500" : "text-zinc-400"}`}>baris dilewati</p>
          </div>
        </div>

        <p className="rounded-lg bg-zinc-50 px-3 py-2 text-xs text-zinc-500">
          Nominal diambil langsung dari kolom <strong>Total Collected</strong> Moka POS. Nama menu disimpan sebagai catatan.
        </p>
        ...
        {skipReasons.length > 0 && (
          <details className="rounded-xl border border-amber-100 bg-amber-50 p-3">
            <summary className="cursor-pointer text-xs font-semibold text-amber-700">
              ⚠️ {willSkip} baris dilewati — klik untuk lihat alasan
            </summary>
        ...
        {willImport === 0 && (
          <p className="rounded-lg bg-red-50 px-3 py-2 text-xs text-red-700">
            ❌ Tidak ada transaksi yang bisa diimpor. Periksa format file.
          </p>
        )}
