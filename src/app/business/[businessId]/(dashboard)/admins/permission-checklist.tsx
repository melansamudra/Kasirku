...
export default function PermissionChecklist({
  ...
  function renderGroup(group: PermissionGroup) {
    return (
      <div key={group.title}>
        <p className="mb-1.5 text-[10.5px] font-semibold uppercase tracking-wide text-zinc-400">{group.title}</p>
  ...
  return (
    <div className="space-y-3">
      {PERMISSION_GROUPS.map(renderGroup)}
      {extraGroups.length > 0 && (
        <>
          <p className="pt-1 text-[10.5px] font-semibold uppercase tracking-wide text-brand-600">
            Per Lokasi (khusus bisnis ini)
          </p>
