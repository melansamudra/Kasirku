...
import type { ImportIngredientsState } from "./actions";

const initialState: ImportIngredientsState = { error: null, result: null };

export default function ImportIngredientsForm({
  action,
}: {
  action: (state: ImportIngredientsState, formData: FormData) => Promise<ImportIngredientsState>;
}) {
    ...
    <form ref={formRef} action={formAction} className="space-y-3">
      ...
      {state.result && !pending && (
        <div className="rounded-lg bg-brand-50 px-3 py-2 text-xs text-brand-700">
          <p>
            ✓ {state.result.created} bahan baru, {state.result.updated} diperbarui
