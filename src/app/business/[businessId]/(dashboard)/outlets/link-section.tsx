...
export default function OutletRequestLinkSection({
  businessId,
  initialSlug,
  regenerateAction,
}: {
  ...
  const url = typeof window !== "undefined" ? `${window.location.origin}/permintaan-resto/${slug}` : "";
  ...
  return (
    <div>
      <div className="flex items-center gap-2 rounded-lg border border-zinc-200 bg-zinc-50 px-3 py-2.5">
        ...
        <Link
          href={`/business/${businessId}/outlets/print-qr`}
          target="_blank"
          className="shrink-0 rounded-lg border border-zinc-200 bg-white px-2.5 py-1 text-[11px] font-medium text-zinc-600 transition-colors hover:border-brand-300 hover:text-brand-700"
        >
          🖨️ Cetak QR
      ...
      <p className="mt-1.5 text-[11px] text-zinc-400">
        Bagikan link ini ke tiap outlet (mis. print jadi poster/QR, atau kirim lewat grup WhatsApp)
        supaya mereka bisa mengajukan permintaan bahan setengah jadi tanpa perlu login.
      </p>
