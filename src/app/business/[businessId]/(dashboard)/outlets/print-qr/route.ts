...
export async function GET(
  ...
  const { data: biz } = await supabase
    .from("businesses")
    .select("name, outlet_request_slug")
    .eq("id", businessId)
    .single();

  if (!biz?.outlet_request_slug) {
    return new NextResponse("Link permintaan resto belum tersedia.", {
      status: 200,
      headers: { "Content-Type": "text/plain" },
    });
  }

  const url = `${SITE_URL}/permintaan-resto/${biz.outlet_request_slug}`;
...
<head>
  <meta charset="UTF-8" />
  <title>QR Permintaan Resto — ${escapeHtml(biz.name)}</title>
  <style>
...
<body>
  <div class="card">
    ...
    <p class="sub">Scan buat ajukan permintaan bahan setengah jadi</p>
