    ...
    <div className="mt-2 rounded-xl border border-zinc-200 bg-zinc-50 p-3">
      ...
      >
        ...
        <button
          ...
          className="flex w-full items-center justify-center gap-1 rounded-lg border border-dashed border-zinc-300 py-1.5 text-xs font-medium text-zinc-500 hover:border-brand-400 hover:text-brand-600"
        >
          + Baris Varian
        </button>

        {state.error && (
          <p className="rounded-lg bg-red-50 px-2 py-1.5 text-xs text-red-600">{state.error}</p>
        )}
        ...
        <div className="flex gap-2">
          ...
          <button
            type="button"
            onClick={() => { setOpen(false); setVariants([{ label: "", price: "", cost: "" }]); }}
