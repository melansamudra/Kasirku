...
export default function ApproveForm({
  ...
  function handleApprove() {
    setError(null);
    setPending(true);
    approvePaymentRequest(businessId, requestId)
      .then((res) => {
        setPending(false);
        if (res.error) {
          setError(res.error);
  ...
  function handleReject() {
    setError(null);
    setPending(true);
    rejectPaymentRequest(businessId, requestId, reason)
      .then((res) => {
        setPending(false);
        if (res.error) {
          setError(res.error);
  ...
  return (
    <div className="mt-4 rounded-xl border border-amber-200 bg-amber-50 p-3 print:hidden">
      <p className="text-[11px] font-semibold text-amber-800">Menunggu Persetujuan Owner</p>
      {!isOwner ? (
        <p className="mt-2 text-[11px] text-amber-700">
          Hanya akun Owner yang bisa menyetujui/menolak pengajuan pembayaran ini.
        </p>
      ) : (
