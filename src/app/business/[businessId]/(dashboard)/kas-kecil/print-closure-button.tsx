...
import { buildPettyCashClosurePrintJobs } from "../../report-print-actions";
import { dispatchPrintJobs } from "@/lib/dispatch-print-jobs";

type State = "idle" | "sending" | "ok" | "error" | "empty";

export default function PrintClosureButton({
  businessId,
  closureId,
}: {
  businessId: string;
  ...
  async function handlePrint() {
    setState("sending");
    try {
      const built = await buildPettyCashClosurePrintJobs(businessId, closureId);
