...
import { rejectOutletRequest } from "./actions";
...
export default function RequestCard({
  ...
  request: {
    ...
    created_at: string;
    items: { id: string; item_name: string; unit: string; qty_requested: number; qty_approved: number | null; value: number | null }[];
  };
  approveAction: (state: ActionState, formData: FormData) => Promise<ActionState>;
}) {
  const router = useRouter();
  const [state, formAction, pending] = useActionState(approveAction, initialState);
  const [rejecting, setRejecting] = useState(false);
  const [reason, setReason] = useState("");
  const [rejectPending, setRejectPending] = useState(false);
  const [rejectError, setRejectError] = useState<string | null>(null);

  const totalValue = request.items.reduce((sum, it) => sum + (it.value ?? 0), 0);

  return (
    <div className="rounded-xl border border-zinc-200 bg-white px-4 py-3.5">
      <div className="flex flex-wrap items-start justify-between gap-2">
        <div className="min-w-0">
          <p className="text-sm font-semibold text-zinc-900">{request.outlet_name}</p>
        ...
        <form action={formAction} className="mt-3 space-y-2">
          ...
          {!rejecting ? (
            <div className="flex gap-2">
              ...
              >
                {pending ? "Memproses…" : "Setujui & Penuhi"}
              </button>
              <button
                type="button"
              ...
              <div className="flex gap-2">
                <button
                  ...
                  onClick={async () => {
                    ...
                    setRejectError(null);
                    const result = await rejectOutletRequest(businessId, request.id, reason);
            ...
            <div key={item.id} className="flex items-center justify-between text-xs text-zinc-600">
              ...
              <span>
                {request.status === "disetujui"
                  ? `${item.qty_approved ?? item.qty_requested} ${item.unit} · ${formatRupiah(item.value ?? 0)}`
                  : `diminta ${item.qty_requested} ${item.unit}`}
              </span>
            </div>
          ))}
          {request.status === "disetujui" && (
            <div className="flex items-center justify-between border-t border-zinc-100 pt-1.5 text-xs font-semibold text-zinc-700">
              <span>Total nilai distribusi</span>
