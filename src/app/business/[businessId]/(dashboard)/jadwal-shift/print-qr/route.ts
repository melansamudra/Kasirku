...
export async function GET(
  ...
  const { data: biz } = await supabase
    .from("businesses")
    .select("name, attendance_qr_slug")
    .eq("id", businessId)
    .single();

  if (!biz?.attendance_qr_slug) {
    return new NextResponse("Link absen selfie belum tersedia.", {
      status: 200,
      headers: { "Content-Type": "text/plain" },
    });
  }

  const url = `${SITE_URL}/absen/${biz.attendance_qr_slug}`;
...
<head>
  <meta charset="UTF-8" />
  <title>QR Absen Selfie — ${escapeHtml(biz.name)}</title>
  <style>
...
<body>
  <div class="card">
    ...
    <p class="sub">Scan buat absen selfie (masuk/pulang)</p>
