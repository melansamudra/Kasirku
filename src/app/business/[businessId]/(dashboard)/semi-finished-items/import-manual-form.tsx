...
import type { ImportManualState } from "./actions";

const initialState: ImportManualState = { error: null, result: null };

export default function ImportManualForm({
  action,
}: {
  action: (state: ImportManualState, formData: FormData) => Promise<ImportManualState>;
}) {
    ...
    <form ref={formRef} action={formAction} className="space-y-3">
      ...
      {state.result && !pending && (
        <div className="rounded-lg bg-brand-50 px-3 py-2 text-xs text-brand-700">
          <p>
            ✓ {state.result.created} BSJ baru, {state.result.updated} diperbarui
