...
import type { ParseState, ConfirmState, BahanResolution, BahanDecision } from "./upload-actions";
...
type DecisionLocal = { action: "existing" | "new"; ingredientId: string; name: string; unit: string; price: number };
    ...
    if (parseState.resolutions) {
      ...
      for (const r of parseState.resolutions) {
        init[r.bahan] = {
          ...
          ingredientId: r.matchedId ?? r.candidates[0]?.id ?? "",
          name: r.bahan,
                        ...
                        <label className="flex items-center gap-2">
                          ...
                          <select
                            ...
                            onChange={(e) =>
                              setDecisions((prev) => ({
                                ...prev,
                                [r.bahan]: { ...prev[r.bahan], ingredientId: e.target.value },
                              }))
                            }
            ...
            action={(fd) => {
              const decisionList: BahanDecision[] = (parseState.resolutions ?? []).map((r) => {
                ...
                return d.action === "existing"
                  ? { bahan: r.bahan, action: "existing", ingredientId: d.ingredientId }
    ...
    <form action={runParse} className="space-y-3">
      <div>
        ...
        <p className="mt-1 text-[11px] text-zinc-400">
          ...
          <a href="/template-resep-bsj" className="font-medium text-brand-600 hover:underline">
