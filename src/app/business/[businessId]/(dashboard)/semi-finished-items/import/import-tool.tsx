...
export default function ImportTool({
  ...
  const [lossFactorPct, setLossFactorPct] = useState("3");

  const group = groups.find((g) => g.itemName === itemName);

  useEffect(() => {
    if (!pending && state.success) {
      // Setelah simpan sukses, biarkan pilihan menu & angka tetap tampil
      // (staf sering cek ulang hasilnya), cuma refresh status "sudah ada".
    }
