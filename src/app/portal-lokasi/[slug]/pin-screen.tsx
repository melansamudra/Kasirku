  ...
  function pressKey(key: string) {
    ...
    if (next.length === 4) {
      startTransition(async () => {
        const result = await loginPortal(slug, businessId, locationId, selected.id, next);
        if (!result.success) {
      ...
      <div className="flex min-h-screen items-center justify-center bg-zinc-900 px-6">
        <div className="w-full max-w-xs">
          <div className="mb-8 text-center">
            ...
            <p className="mt-1 text-sm text-zinc-400">{locationName} — pilih nama untuk mulai</p>
          </div>

          <div className="space-y-2">
            {employees.length === 0 ? (
              <p className="rounded-xl border border-dashed border-zinc-700 px-4 py-6 text-center text-xs text-zinc-500">
                Belum ada staf dengan PIN di lokasi ini. Minta admin set PIN dulu di halaman Staf.
              </p>
            ) : (
              employees.map((e) => (
                <button
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-zinc-900 px-6">
      <div className="w-full max-w-xs">
        ...
        >
          Bukan {selected.name}? Ganti staf
        </button>
      </div>
    </div>
  );
