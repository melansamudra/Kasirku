...
import { submitPurchaseRequestPortal } from "./actions";
...
type CartRow = {
  key: string;
  itemId: string; // MasterItem.id, atau NEW_ITEM_VALUE
  newItemName: string;
  unit: string;
  qtyOrdered: string;
  currentStock: string;
...
export default function OrderFormClient({
  ...
  async function handleConfirmSend() {
    ...
    setPending(true);
    const res = await submitPurchaseRequestPortal(purchaseRequestSlug, businessId, locationId, note, reviewItems);
    setPending(false);
      ...
      <div className="mt-4 space-y-4">
        <div className="rounded-xl border border-amber-200 bg-amber-50 p-3">
          ...
          <p className="mt-0.5 text-[11px] text-amber-700">
            Pastikan barang &amp; qty di bawah sudah benar. Kalau ada yang salah, tekan &quot;Kembali&quot;
            buat edit lagi.
          </p>
        </div>
          ...
          return (
            <div key={row.key} className="rounded-xl border border-zinc-200 p-3">
              ...
              <div className="mt-2 grid grid-cols-2 gap-2">
                <div>
                  <label htmlFor={`${formId}-qty-${row.key}`} className="mb-1 block text-[11px] text-zinc-500">
                ...
                <div>
                  <label htmlFor={`${formId}-stock-${row.key}`} className="mb-1 block text-[11px] text-zinc-500">
