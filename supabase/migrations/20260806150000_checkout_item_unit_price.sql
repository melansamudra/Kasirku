-- Tambahkan dukungan unit_price opsional per item di p_items JSON.
-- Dipakai saat item memiliki opsi/modifier yang mengubah harga dasar.
-- Kalau unit_price tidak dikirim atau 0, fallback ke v_product.price.
...
begin
  ...
  loop
    ...
    -- Pakai unit_price dari client jika dikirim (mis. ada opsi yg ubah harga),
    -- fallback ke harga produk di DB.
    ...
    insert into public.transaction_items (
      ...
      v_product.category,
      v_unit_price, v_product.cost, v_qty, v_disc, v_disc_type, v_note
    );
