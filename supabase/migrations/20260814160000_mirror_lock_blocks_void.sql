-- "Kunci Bulan" di halaman Mirror > Rekap & Kunci sebelumnya cuma
-- memblokir toggle 👁 (insert/delete di mirror_visible_transactions) lewat
-- trigger private.check_mirror_tx_month_lock(). Transaksinya sendiri masih
-- bisa di-void kapan saja lewat void_transaction_item / owner_void_transaction
-- / void_transaction -- angka rekap yang katanya "terkunci" bisa diam-diam
-- berubah tanpa proteksi apapun begitu ada void di bulan itu.
--
-- Owner minta: kunci harus benar-benar mengunci -- void ditolak sampai
-- kuncinya dibuka lagi secara sadar (bukan sekadar peringatan setelah
-- kejadian).

create or replace function private.assert_mirror_month_unlocked(
  p_business_id uuid,
  p_date timestamptz
)
returns void
...
declare
  v_month date;
begin
  v_month := date_trunc('month', p_date at time zone 'Asia/Jakarta')::date;
  if exists (
    select 1 from public.mirror_month_locks
    where business_id = p_business_id
      and month_year = v_month
  ) then
    raise exception 'Bulan % sudah dikunci untuk mirror. Buka kunci terlebih dahulu di halaman Mirror > Rekap & Kunci.',
...
-- void_transaction (staff/manager PIN path) -----------------------------------
