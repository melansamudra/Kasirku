-- Fitur "Kelola Produk" untuk bisnis cost-control (mis. Dapur Produksi
-- sebagai toko sendiri, terpisah dari Llauk Nusantara) -- produk yang dijual
-- di sini adalah Bahan Setengah Jadi yang diberi harga jual, bukan produk
-- dengan resep bahan baku biasa. Default sengaja mati (false/null) supaya
-- bisnis cost-control yang sudah ada (Llauk Nusantara) tidak terdampak sama
-- sekali -- sidebar & checkout mereka tetap seperti sebelumnya.

alter table public.businesses
  add column sell_products_enabled boolean not null default false;

alter table public.products
  add column semi_finished_item_id uuid references public.semi_finished_items (id) on delete set null;

create index products_semi_finished_item_id_idx on public.products (semi_finished_item_id);

-- Checkout: produk yang terhubung ke Bahan Setengah Jadi mengurangi stok di
-- semi_finished_item_location_stock (lokasi bertanda is_production milik
-- bisnisnya), BUKAN products.stock -- supaya cuma ada satu angka stok yang
-- jadi acuan (dari Produksi), tidak ada dua angka yang harus disinkron
-- manual. Produk tanpa semi_finished_item_id (bisnis fnb/retail biasa)
-- jalan persis seperti sebelumnya, tidak ada perubahan perilaku.
...
grant execute on function public.checkout_transaction(
  uuid, uuid, jsonb, jsonb, numeric, text, uuid, uuid[], uuid, text
) to authenticated;
