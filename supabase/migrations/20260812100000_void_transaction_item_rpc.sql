-- Void satu item dalam transaksi yang sudah selesai.
-- Dampak:
--   1. transaction_items.voided = true
--   2. transactions.total (dan komponen-nya) berkurang sebesar kontribusi item
--   3. Jurnal pembalik diposting → kas harian mencatat kas keluar (refund)
--   4. Stok produk & bahan baku dikembalikan
--
-- Kontribusi item ke total dihitung proporsional terhadap diskon order,
-- service charge, dan pajak pada transaksi induk — sama dengan cara
-- checkout_transaction menghitung total item per item.
...
begin
  ...
  -- Hitung kontribusi item ke subtotal_raw dan disc-nya sendiri
  ...
  -- Porsi diskon order (proporsional terhadap item_net dibanding seluruh item_net)
  ...
  -- Kontribusi item ke subtotal (setelah semua diskon)
  v_item_subtotal := v_item_net - v_order_disc_portion;

  -- Porsi service (proporsional terhadap subtotal)
  ...
  -- Porsi pajak (proporsional terhadap subtotal + service)
  v_tax_portion := case
  ...
  -- Kurangi total transaksi sebesar kontribusi item
  ...
  -- Kembalikan stok bahan baku (dari resep)
  ...
  -- Buat jurnal pembalik (sama polanya dengan owner_void_transaction)
  v_journal_lines := '[]'::jsonb;
...
grant execute on function public.void_transaction_item(uuid, uuid, uuid, text, uuid) to authenticated;
