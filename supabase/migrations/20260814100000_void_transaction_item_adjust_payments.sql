-- void_transaction_item sebelumnya mengurangi transactions.total (dan
-- shifts.total_sales untuk shift yang sudah tutup) tapi tidak pernah
-- menyentuh transaction_payments. Akibatnya "Tunai"/"Kas diharapkan di
-- laci" di halaman shift tetap menampilkan jumlah SEBELUM item di-void,
-- padahal di lapangan kasir langsung mengembalikan selisihnya ke pelanggan
-- saat itu juga (dikonfirmasi 2026-08-14).
--
-- Perbaikan: kurangi transaction_payments.amount proporsional terhadap
-- kontribusi item ke total transaksi (pola proporsional yang sama dengan
-- perhitungan diskon/service/pajak di fungsi ini). Untuk shift yang sudah
-- ditutup, kolom snapshot cash_sales/non_cash_sales/expected_cash/difference
-- juga disesuaikan — total_sales sudah dibetulkan di migrasi 20260812120000,
-- ini melengkapi bagian yang terlewat.
