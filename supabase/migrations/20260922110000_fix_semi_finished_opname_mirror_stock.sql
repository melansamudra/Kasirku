...
-- lihat migrasi semi_finished_ingredient_mirror) sebenarnya TIDAK PERNAH
-- diisi stoknya lewat semi_finished_item_location_stock -- stok yang
-- beneran dipotong checkout/resep ada di ingredient_location_stock milik
-- kembarannya. Akibatnya "Stok Data" yang dibaca di sini untuk BSJ
-- bermirror selalu 0 (kolom yang salah), bikin selisih opname keitung
-- ngawur. Sekarang baca dari kembarannya kalau ada, baru fallback ke
-- semi_finished_item_location_stock kalau memang tidak ada kembaran.
...
begin
  ...
  loop
    ...
    select id, name, unit, ingredient_id into v_semi
    from public.semi_finished_items
    where id = (v_row ->> 'id')::uuid and business_id = v_business_id and deleted_at is null;
    if not found then
      continue;
    ...
    if v_semi.ingredient_id is not null then
      select stock into v_system_stock
      from public.ingredient_location_stock
      where business_id = v_business_id and location_id = p_location_id and ingredient_id = v_semi.ingredient_id;
    else
      select stock into v_system_stock
