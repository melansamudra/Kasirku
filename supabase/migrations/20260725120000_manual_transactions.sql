-- Manual transaction entry: lets the owner record a sale directly from the
-- backoffice (no cashier PIN session, no open shift required) with a
-- backdated date — for sales that were never rung up through the POS.
-- Mirrors checkout_transaction's core logic (supabase/migrations/
-- 20260713100000_offline_checkout_idempotency.sql) but deliberately not
-- refactored into it — that function is the most-revised, most-trafficked
-- path in the app, too risky to touch for this.
...
begin
  ...
  -- Same journal shape as checkout_transaction: Kas & Bank debited for the
  -- full total, (subtotal + service) credited to Pendapatan, PPN collected
  -- credited to PPN Keluaran (liability), HPP/Persediaan for recipe cost.
  v_journal_lines := '[]'::jsonb;
  if v_total > 0 then
