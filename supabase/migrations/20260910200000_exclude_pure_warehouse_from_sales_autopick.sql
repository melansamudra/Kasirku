-- Lanjutan 20260910160000/170000/180000. Kasus baru muncul begitu bisnis
-- punya Gudang (is_default_purchase=true, is_production=false, murni
-- staging -- lihat migration 20260910190000 & Kota Baru) -- kalau bahan
-- BARU dibeli ke Gudang dan belum sempat ditransfer ke Kitchen/Bar,
-- "auto-pick lokasi" yang ada (1 lokasi ketemu -> langsung dipakai) bisa
-- salah motong stok Gudang langsung waktu ada penjualan -- padahal Gudang
-- seharusnya cuma transit, bukan tempat jualan.
--
-- Fix (arahan user 2026-09-10): lokasi "Gudang murni" (is_default_purchase
-- true DAN is_production false) DIKECUALIKAN dari auto-pick -- kalau bahan
-- CUMA ada stoknya di situ, tetap dianggap ambigu/tidak ketemu, jatuh ke
-- fallback lokasi produksi. SENGAJA cek is_production JUGA (bukan cuma
-- is_default_purchase) -- Kitchen Llauk kebetulan is_default_purchase=true
-- DAN is_production=true sekaligus (dobel fungsi), jadi TIDAK boleh ikut
-- kena exclude ini, supaya Llauk (sudah aktif pakai flag ini) 0% berubah
-- perilakunya.
--
-- Sekalian: kalau jatuh ke fallback (baik karena exclude di atas, ATAUPUN
-- kasus ambigu/kosong yang sudah ada sebelumnya), stok lokasi tujuan
-- SEKARANG BOLEH JADI MINUS (tidak dilantai ke 0 lagi) -- sengaja, sebagai
-- sinyal visual "bahan ini belum sempat ditransfer dari Gudang" yang
-- kelihatan di Kartu Stok/Rekonsil, bukan didiamkan/disembunyikan. Jalur
-- normal (1 lokasi non-Gudang jelas ketemu) TETAP dilantai ke 0 seperti
-- sebelumnya -- cuma jalur fallback yang berubah.
...
begin
  ...
  loop
    ...
    loop
      ...
      if v_business.location_scoped_sales_enabled then
        select count(distinct ils.location_id) into v_location_count
        from public.ingredient_location_stock ils
        join public.stock_locations sl on sl.id = ils.location_id
        where ils.business_id = p_business_id and ils.ingredient_id = v_recipe.ingredient_id
          and ils.stock > 0
          and not (sl.is_default_purchase and not sl.is_production);

        if v_location_count = 1 then
          select ils.location_id into v_consumption_location_id
          from public.ingredient_location_stock ils
          join public.stock_locations sl on sl.id = ils.location_id
          where ils.business_id = p_business_id and ils.ingredient_id = v_recipe.ingredient_id
            and ils.stock > 0
      ...
      if v_consumption_location_id is not null then
        ...
        v_loc_stock_after := case
          when v_used_fallback then v_loc_stock_before - v_recipe.qty * v_qty
          else greatest(0, v_loc_stock_before - v_recipe.qty * v_qty)
        end;
...
-- create_manual_transaction() -- perubahan sama persis.
  ...
  loop
    ...
    loop
      ...
      if v_consumption_location_id is not null then
        ...
        v_loc_stock_before := coalesce(v_loc_stock_before, 0);
        v_loc_stock_after := case
          when v_used_fallback then v_loc_stock_before - v_recipe.recipe_qty * v_qty
          else greatest(0, v_loc_stock_before - v_recipe.recipe_qty * v_qty)
        end;
...
-- import_esb_sales_bulk() -- perubahan sama persis.
