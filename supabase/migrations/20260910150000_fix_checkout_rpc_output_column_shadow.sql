-- Hotfix: "column reference \"transaction_id\" is ambiguous" saat checkout
-- produk yang punya resep (product_recipes). Sejak checkout_transaction()
-- ditulis ulang di 20260901100000_products_semi_finished_link.sql, RETURNS
-- TABLE-nya punya kolom output literal bernama `transaction_id` -- itu jadi
-- variabel implisit yang berlaku di seluruh body function. Baris
-- "on conflict (transaction_id, ingredient_id)" saat insert ke
-- transaction_ingredient_consumption jadi ambigu: Postgres tidak tahu apakah
-- itu merujuk ke kolom tabel transaction_ingredient_consumption atau ke
-- output parameter function. Baru kena sekarang karena baru Mie Kota yang
-- checkout produk BERRESEP lewat versi function ini (produk tanpa resep tidak
-- pernah masuk loop ini, jadi lolos selama ini).
--
-- Fix: `#variable_conflict use_column` di awal body -- perintah eksplisit ke
-- plpgsql supaya, kalau ada nama ambigu antara kolom tabel & variabel/output
-- parameter, SELALU pilih kolom tabel. Ini aman di sini karena di semua titik
-- ambigu, makna yang benar memang kolom tabel (variabel lokal yang dimaksud
-- sudah dinamai v_transaction_id, beda nama, tidak pernah collide).
