-- Perbarui checkout_transaction agar membaca field "note" opsional dari
-- setiap elemen p_items dan menyimpannya ke transaction_items.note.
-- transaction_items.note sudah ada sejak migrasi awal — hanya INSERT-nya
-- yang sekarang juga mengisi kolom itu.
-- Signature fungsi tidak berubah; GRANT tidak perlu diulang.
...
create or replace function public.checkout_transaction(
  p_business_id    uuid,
  p_cashier_id     uuid,
  p_items          jsonb,   -- array of {product_id, qty, disc, disc_type, note?}
