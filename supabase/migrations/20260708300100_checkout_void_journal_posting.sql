-- Mini-ERP accounting module, part 2: auto-post journal entries from the two
-- highest-volume money flows — sales (checkout_transaction) and voids
-- (void_transaction). Everything else about these RPCs is unchanged; the
-- only addition is a call to private.post_journal() near the end.
...
begin
  ...
  -- plus HPP / Persediaan if this sale had recipe-based cost data.
  if v_total > 0 then
