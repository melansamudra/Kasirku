-- Module 13: diskon (per item & per order), PPN, dan biaya layanan di
-- checkout. Kolom transactions (total_item_disc, order_disc_amt, service,
-- tax) dan businesses (tax_enabled/tax_rate/service_enabled/service_rate)
-- sudah ada sejak awal — migrasi ini mengisi mereka dengan angka sungguhan.
--
-- Urutan hitung (mengikuti aplikasi lama):
--   diskon item -> diskon order -> layanan dari subtotal -> PPN dari
--   (subtotal + layanan). Semua diskon divalidasi & di-clamp server-side.

alter table public.transaction_items
  add column if not exists disc numeric(12, 2) not null default 0,
  add column if not exists disc_type text not null default 'pct'
    check (disc_type in ('pct', 'amt'));

-- Signature lama harus di-drop dulu; kalau tidak, versi 5-argumen dan
-- 7-argumen hidup berdampingan dan pemanggilan lama jadi ambigu.
drop function if exists public.checkout_transaction(uuid, uuid, jsonb, text, numeric);
...
grant execute on function public.checkout_transaction(uuid, uuid, jsonb, text, numeric, numeric, text) to authenticated;
